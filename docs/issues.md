...
### 25. The crossing card at the largest accessibility size — below the fold, and it isn't known whether the reader can get to it

At `accessibility-extra-extra-extra-large`, in both light and dark (`/tmp/ax5-crossing-light.png`, `/tmp/ax5-crossing-dark.png`, from the phase 12 verification pass), the crossing card shows the first note in full, the hairline, and then the second note cut off after its first line — `Attention is a`. Nothing below that point is on screen: not the rest of the second note, not its source line, not the foot (`29 days apart` and `[x] not related`). What *is* on screen at that point is the paging progress bar and `↑ swipe up for next`, which pages to the next card in the day's set, not further into this one.

**The crossing card is the first card in this app designed to hold two full notes.** Every other `ReviewCard` holds one, and at AX5 what falls below the fold there is more of the *same* note — annoying, but the reader can keep scrolling the one thing they're already reading. Here what's below the fold is the *other half of the idea the card exists to show*, and the card's only action. A reader who can't reach `[x] not related` at this text size has a card with no working action on it at all.

**`CrossingCard.body` and `ReviewCard.body` use the identical construction** — `GeometryReader { ScrollView { card.frame(minHeight: proxy.size.height).onGeometryChange(...) } }.scrollDisabled(content <= proxy.size.height + 0.5)` — and `onGeometryChange` is attached after `.frame(minHeight:)` in both, so `content` measures the *natural* height once it exceeds the frame's floor. At AX5 the crossing card's combined content should exceed `proxy.size.height`, which should evaluate `scrollDisabled` false and enable the inner scroll, exactly as it does on `ReviewCard`'s long notes since phase 11. Read side by side, the two files do not differ in a way that would disable the scroll on this card specifically.
